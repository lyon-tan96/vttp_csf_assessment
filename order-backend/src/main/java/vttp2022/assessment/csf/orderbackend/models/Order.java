...
public class Order {
	...
	public static Order create(String json) {
		...
		// order.isThickCrust(data.getBoolean("thickCrust"));
		order.setToppings(data.getValue("toppings"));

		JsonArray array = data.getJsonArray("toppings");

		for (int i=0; i < array.size(); i++) {
			order.addTopping(array.getString(i));
		}
